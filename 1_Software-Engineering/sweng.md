...
* 하향식 설계
  ...
  * **레벨이 낮은 데이터 구조**의 세부사항은 **설계 초기 단계**에서 필요
  * **HIPO 차트**
...
* 프로토타입 모델
  ...
  * 단기간에 프로토타입을 제작해야하기 때문에 효율성과 무관한 언어나 알고리즘이 사용될 수 있음
* 나선형 모델

  * 대규모 시스템 개발
  * 프로젝트 수행시 발생하는 위험을 관리하고 최소화하기 위한 방식
  * **프로토타이핑 모델을 기반**으로 하고 있으며, **위험분석(Risk Analysis)** 단계가 추가되어 반복적으로 개발
  * **계획 수립 - 위험 분석 - 개발 및 검증 - 고객평가** 순으로 반복
* V 모델
  * 각 개발단계별로 상응하는 테스트 작업이 존재
  * 작업의 **검증에 중점**을 두고 신뢰성 있는 소프트웨어를 개발
* UP
...
* 같은 입력자료를 이용하거나 **동일 출력 데이터**를 만들어내는 경우

* **동일한 속성을 사용**하는 기능을 묶어놓은 경우
