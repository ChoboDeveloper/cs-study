...
> 동기 통신과 비동기통신의 가장 큰 차이점은 **클럭(clock)의 유무**

* 동기 통신은 클럭이라는 신호를 계속 주기적으로 보내고, 클럭과 클럭 사이에 사용자가 정해놓은 수만큼의 문자열을 한 블록으로 하여 전송한다. A와 B 사이에는 지속적으로 클럭신호가 오가고 있으며, 데이터의 전송이 없더라도 클럭신호는 계속 주고받는다. A와 B는 서로 데이터만 보내면 되기 때문에 전송속도가 빠르다.
* 비동기통신은 평상시 클럭 신호를 보내지 않는다. 대신에 보낼 데이터가 있을 때 Start bit를 보낸 후 데이터를 보내고 다 보냈으면 Stop bit를 보낸다. 따라서 동기 통신에 비해서는 전송속도가 느릴 수밖에 없다.
...
* 열잡음은  온도에  대한  함수  형태로서  도체  내에서  온도에  따른  전자의  운동량  변화에  기인한다.  빛의  모든  색깔을  합하면  백색이  되는데  열잡음도  모든  주파수에  골고루  분포하기  때문에  백색잡음(white noise)이라고  불린다.
* 열잡음은  제거될  수  없는  잡음이므로  이  잡음만을  고려하여  계산한  전송매체의 용량이  이론적  최대  전송용량이  된다.

**누화**

* 누화는  전화 통화  중에  다른  사람의  말이  들리는  혼선과  같은  현상을  말하며  열잡음에  비하여  크기가  작은  편이다.
* 누화는  인접한  경로의  신호가  상호 간섭하여  비정상적으로  결합된  경우에  발생하는데  이러한  누화를  방지하기 위하여  전화선을  서로  꼬아  트위스트  시켜  사용한다.
  * 인접 케이블의 전기적 신호결합 또는 다중화전송 동축 케이블에서 발생

**상호변조 잡음**

* 상호변조  잡음은  서로  다른  주파수  성분들이  동일한  전송매체를  공유할  때에  유발될  수  있다. 각  주파수  성분들이  배수  값으로  나타나는  것이  아니라 각  주파수의  합이나  차의  신호로  생성될  때  발생하는  잡음이  바로  상호변조 잡음이다.
* 예를  들어 3㎑와 4㎑의  주파수  성분을  가지는  신호가  전송매체를 통과하면서 7㎑와 1㎑에  해당하는  새로운  주파수를  생성할  수  있는데  이  경우  이미  이  주파수를  사용하는  다른  신호와  간섭현상을  일으킬  수  있게  된다.

**충격 잡음**

* 충격 잡음은  어느  정도  예측  가능한  다른  잡음들과는  달리  예측할  수  없는 외부적  요인,  즉  번개,  자기적  충격,  기계적인  충격,  통신시스템의  결함  등으로  발생한다.
* 충격 잡음은  시스템  기술자들이  이에  대해  적절한  조치를  취할 수도  없는  잡음으로서  불규칙적인  진폭을  가지며  짧은  순간  동안  다소  큰  세기로  발생한다.
* 예를  들어  음성통화의  경우에는 0.01초  동안에  순간적인  잡음이  발생해도  음성을  인지할  수  없을  정도가  아니지만 9,600bps의  속도로 디지털  데이터를  전송하는  경우에는 0.01초의  충격 잡음으로 100비트  정보가 유실되게  된다.
...
* **60GHz 주파수에서 최대 7Gbps**의 데이터 전송률을 제공할 수 있는 매우 빠른 표준이지만 그 대가로 전송 거리가 짧다. 이 속도는 클라이언트 기기가 AP에서 3.3미터 이내에 위치한 경우에만 달성 가능하다.
* **802.11ay**
  * 802.11ad의 개선, 초고속 WiFi 네트워크 기술
  * 차세대 60GHz로도 알려진 표준으로, **60GHz 주파수 내에서 20Gbps** 이상의 최대 처리량을 제공하고(현재 802.11ad는 최대 7Gbps) 거리와 안정성도 개선하는 것을 목표로 한다.
...
> [디지털 to 디지털 전송 (1) - 라인 코딩 개요 (tistory.com)](https://gusdnd852.tistory.com/271?category=746573)

* 2진 (Binary) 데이터를 디지털 신호로 만드는 것을 라인 코딩
...
* 8B10B
  * 8bit 단위를 10bit의 코드로 변환
  * 비트 동기화에 유리하다.
  * 0과 1 발생비율을 평균적으로 같게한다.
...
*  애플리케이션, 프레젠테이션, 세션 계층: 데이터(Data)
* 트랜스포트 계층 : 세그먼트(Segment)
* 네트워크 계층 : 패킷(Packet) 혹은 데이터그램(Datagram)
* 데이터링크 계층 : 프레임(Frame)
* 물리 계층 : 비트(Bit)
...
> (XX)Base(YY)
>
> 100base5 : 100Mbps, 기저대역전송, 500m

- **XX** : 인터페이스의 정격 속도를 **Mbps**로 표기한다.

- BASE/BROAD : 이더넷이 **기저대 신호나 광대역 신호**를 사용하고 있음을 나타낸다.

- - 10BROAD36만 유일하게 광대역 신호를 사용하기 때문에 다른것은 모두 BASE 즉 기저대 신호로 표기한다
  - 기저대 신호란 변조과정 없이 전송하는 신호

- **YY** : 네트워크 세그먼트의 근사 또는 정격 최대 길이를 1**00미터 단위**로 나타낸다.

**현재 표기법**

> **(XX)Base-(YZ)**
>
> 100BASE-T4 의 경우 4쌍을 이용하는 이더넷, 100BASE-T1은 단선 BroadR-Reach의 표준 명칭(maybe)

- XX : **Mbps**로 표기한 인터페이스 속도, 이 속도는 기가비트 이더넷 속도까지 표현 할 수 있다.
  - 10, 100, 1,000이 사용되는데 기가비트 일 경우 숫자가 커지는것을 막기위해 10G, 100G 등으로 표시된다.

- BASE : 이전과 마찬가지로 기저대 신호를 사용하는 인터페이스를 나타낸다.
- Y : 물리 계층이 사용하는 매체 종류를 나타내는 두 개의 문자이다.
  - **일반 및 고속 이더넷의 경우 "Y"**는 **섬유(fiber)의 "F"**나 **꼬임쌍선 케이블 "T"**, 꼬임쌍선과 섬유 이형을 구분하기 위하여 "Z"가 사용된다. 
  - 10BASE-FB, 10BASE-FL, 10BASE-FP는 섬유를 통한 일반 이더넷의 다른 이형이다.

- Z : 꼬임쌍선 이형의 경우에 쌍(pair)의 개수를 나타낸다. 
