...
* 인터넷 전자우편 표준 프로토콜
* 포트 25, 응용계층
* **SMTP**는 **송신용** 프로토콜 / **POP3, IMAP**는 **수신용** 프로토콜
...
* PGP(Pretty Good Privacy)
  ...
  * 구현이 용이함
  * IDEA와 RSA를 조합하여 만들었다. IDEA를 세션키 암호화, RSA를 인증을 위한 전자서명에 사용한다.
  ...
  * WOT(Web of Trust) 사용, 수평적 구조
  ...
  | 기능                  | 알고리즘                         | 설명                                                         |
  | :-------------------- | :------------------------------- | :----------------------------------------------------------- |
  | 기밀성(메시지 암호화) | IDEA, Triple-DES, CAST, RSA      | 송신자가 생성한 일회용 세션키로 알고리즘을 이용해 메시지 암호화.<br>수신자의 공개키로 RSA를 이용해서 세션키를 암호화하고 메시지에 첨부. |
