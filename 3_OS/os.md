...
* 구조
  ...
  <img src = "https://user-images.githubusercontent.com/75229881/170479886-49d8a0dc-28bf-4979-a4fa-7cb6eccdfa86.png" width="30%">
  ...
  * **Process Accounting Information** : CPU를 사용한 시간, CPU 할당 시간 등
...
> 각종 중간값은 다음 실행할 명령어인 `program counter`와 연산이 저장된 `CPU register`를 말하는 것이다.
>
...
* 상태전이
  ...
  * Block - 실행중이던 프로세스가 외부 요인(I/O등)에 의해서 자원을 빼앗기는 상태
...
* CPU가 찾고자하는 주기억장치 메모리가 캐시 메모리 어디에 있는지 **빠르게 검색할 수 있게 사용하는 메모리** = **CAM**(Content Addressable Memory)
* 매핑 테이블을 사용합니다.
* 외부 인자와 내용을 비교하기 위한 **병렬 판독 회로가 필요**하여 추가적인 비용을 요구합니다.
...
<img src = "https://user-images.githubusercontent.com/75229881/170482357-298b251a-fd4b-419b-b723-4936fb1dc8e2.png" width="50%">
...
* 메모리 공간이 고정되어있기 때문에 초기 정해진 분할공간보다 더 큰 프로세스를 적재할 수 없다.
* 하나의 분할에는 하나의 프로그램만 적재 가능하므로 **외부 단편화**와 **내부 단편화**이 발생

**가변분할**

* 메모리에 적재되는 프로세스 크기에 따라 분할의 크기, 개수가 동적으로 변하는 방식
* **메모리 통합** 등의 부가적인 작업이 필요해 메모리 관리가 복잡
* 가변 분할 방식에서 프로세스의 크기보다 분할의 크기보다 크게 할당하기 때문에 **내부 단편화**는 발생하지 않음
* 하지만 이미 메모리에 존재하는 프로그램이 종료되고, 메모리 크기보다 더 작은 크기의 새로운 프로세스가 할당될 때 **외부 단편화**가 발생
* 또한, 프로세스를 어떤 위치에 올릴지 결정하기 위한 **동적 메모리 할당 문제**가 있음
...
* **직접 매핑**
  * 페이지 테이블 전체가 물리 메모리에 존재하는 방식.

* **연관 매핑**
  * 페이지 테이블 전체를 스왑 영역에서 관리하는 방식
  * 물리 메모리의 여유 공간이 작을 때 사용하는 방식
  * 모든 페이지 테이블을 저장 장치의 스왑 영역에 저장하고 그 중 일부만 물리 메모리에 가지고 있음
  * 페이지의 일부만 무작위로 가져옴. 주소 변환시 물리 메모리 내의 페이지 테이블을 다 검색해야 해서 시간 낭비가 발생할 수 있음.

* **직접-연관 매핑**
  * 연관 매핑의 문제를 개선한 방식. 모든 페이지 테이블을 스왑 영역에서 관리하고 일부만 물리 메모리로 가져오는 것은 동일. 페이지 테이블을 일정한 집합으로 자르고, 자른 덩어리 단위로 물리 메모리에 가져와서 시간을 단축.

* **역매핑**
  * 앞의 방식들과 반대로 테이블을 구성. 앞의 매핑은 **페이지 번호를 기준으로 테이블을 구성**하지만, 역매핑은 **물리 메모리의 프레임 번호를 기준으로 테이블을 구성**. 프로세스의 수와 상관없이 테이블은 하나만 존재하고, 테이블의 크기가 작아진다는 것이 장점.
  * 기존 방식이 page 번호를 통해 frame 번호를 찾은 것과 반대로 **역매핑은 역 페이지 테이블을 만들어서 CPU에서 참조하는 주소와 PID 조합으로 Page ID를 만들어 페이지 테이블에서 Page ID를 검색**한다. Page ID를 발견하면 해당 frame 번호를 논리 주소 공간으로 매핑한다.
  * 시스템 전체에 **하나의 페이지 테이블**만 둔다. 페이지 테이블 인덱스는 프레임 번호이고, 페이지 테이블 내용은 프로세스 번호(Process ID)와 페이지 번호이다. 페이지 테이블은 훨씬 적은 용량을 차지하지만, **테이블을 검색하는데 시간이 오래 걸린다는 단점**이 있다.
...
> 주 기억장치(RAM)으로 불러오는 과정을 swap-in, 보조 기억장치로 내보내는 과정을 swap-out 이라 한다. swap에는 큰 디스크 전송시간이 필요하기 때문에 메모리 공간이 부족할때 Swapping 한다.
...
* 세그먼트는 크기고정 x, 가변적이라 동적 메모리 할당을 해야함. -> **외부단편화**가 발생할수있다
...
* 기타
  * 고급언어를 이용하여 알고리즘을 쉽게 표현 가능

**RISC**

* 명령어의 개수가 적으며 고정 길이를 가진다.
* 단순한 컴파일러, 회로구성 단순
* 단순 명령이 많기 때문에 다수의 레지스터가 필요하고 하드와이어(Hard-wired)방식으로 제어
  * 실행빈도가 적은 하드웨어를 제거하여 자원이용률 극대화
...
![image](https://user-images.githubusercontent.com/75229881/164452792-5be256f2-d303-46c4-a6a7-7df884c74fa9.png)
...
* 해결방안
  ...
  * 우선순위 상속기법
    ...
    * 이렇게 하면, task2에 의해 선점되지 않고 우선순위 역전 없이 각 작업이 완료된다.
    ...
    예시)
    task2이 S1 (세마포어1) 을 획득한 후 임계 영역을 실행하던 중 task1이 실행되고, 
    task1이 실행 중에 S2 (세마포어2) 를 요청하고 획득한 후, S1을 요청하게 되면 task2를 기다리게 됨. 
  ...
  * 우선순위 올림기법
    ...
    * 세마포어/뮤텍스를 잠근 스레드는 다른 스레드에 의해 선점되지 않고 종료까지 수행된다.
    ...
    즉, 구현이 복잡하므로 결국 잘 안 쓰이고, 상속기법이 주로 쓰인다.
    ```
...
* 연속할당
  ...
  * 단점
    * 파일을 할당하고 지우고를 반복하다보면 중간 중간에 빈 공간(hole)이 생기는데 연속 할당은 연속된 공간을 찾아야 하기 때문에 이전 메인 메모리 할당에서 살펴본 것과 같이 **외부 단편화** 문제가 발생

* 연결할당

  * 연속적으로 할당하는 것이 아니라 **링크드리스트(linked list)**와 같은 방식으로 파일
  * 각 블록의 마지막에 **주소를 저장하는 포인터 공간(4bytes)**이 존재하며, 여기서 다음 블록을 가리키고 있다. 마지막 블록의 포인터 공간에는 끝임을 나타내는 값이 저장되어 있다.
  * 단점
...
* 색인할당
  ...
  * 색인 할당 역시 연결 할당과 같이 데이터를 랜덤한 블록 번호에 할당하지만 할당된 블록 번호(포인터)를 하나의 블록에 따로 저장한다. 이러한 블록을 **인덱스 블록**이라고 부르며, 파일 당 하나의 인덱스 블록이 존재
