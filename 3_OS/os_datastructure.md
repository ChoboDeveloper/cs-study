...
* 가장 상단의 노드를 '**루트 노드(Root Node)**', 중간 노드들을 '**브랜치 노드(Branch Node)**, **내부노드**', 가장 아래 노드들을 '**리프 노드(Leaf Node), 외부노드**'라고 한다. 
* 인덱스와 실제 데이터가 저장된 데이터는 따로 관리되는데, 인덱스의 **리프 노드는 항상 실제 데이터 레코드를 찾아가기 위한 주소 값**을 가지고 있다.
* **참고사항**
