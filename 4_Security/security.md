...
* SARA는 SATAN이 업데이트가 되지 않는 상황에서 SATAN을 기반으로 개발한 취약점 분석도구로써 네트워크 기반의 컴퓨터, 서버, 라우터 IDS에 대해서 취약점을 분석한다. 
* 유닉스 플랫폼에서 동작하고 HTML 형식의 보고서 기능이 있다.
...
* 시스템 내부에 존재하는 취약성을 점검, 유닉스 플랫폼에서 동작하고 취약한 패스워드 체크

**Nessus**

* 클라이언트 - 서버 구조로 클라이언트를 취약점 점검, 유닉스 플랫폼에서 동작 
